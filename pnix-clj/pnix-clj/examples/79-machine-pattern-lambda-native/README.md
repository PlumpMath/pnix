# 79. Machine pattern lambda native

## 무엇을 보여주나

M7d에서 abstract machine이 pattern lambda를 static refusal이 아니라 native control path로 실행하는 것을 보여준다. default thunk, missing/extra guard, ellipsis, `@as`, `builtins.map` 안의 pattern lambda까지 evaluator와 machine이 같은 결과를 낸다.

## plain Clojure의 한계

Clojure map destructuring은 extra key를 허용하고 missing required는 host exception 형태로 흘러간다. derived machine의 `:pattern-bind` frame, Nix held reason, evaluator와의 lane agreement가 없다.

## pnix-clj 방식

`machine/eval-source`가 pattern closure argument를 WHNF로 평가한 뒤 machine control 안에서 binding/default/body를 실행하고, evaluator와 comparable result를 맞춘다.

## 어디에 쓰나

machine이 단순 lambda를 넘어 Nix function argument semantics까지 먹었는지 확인하거나, generated pattern-heavy source를 회귀 고정할 때 쓴다.
...
(assert (= 5 (clojure-pattern {:x 3 :z 100})))
...
비교하면, limit 파일은 host destructuring 결과/예외만 본다. pnix-clj 파일은 pattern lambda source를 evaluator와 derived machine 양쪽에서 실행해 M7d native pattern semantics를 검증한다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 79. Machine pattern lambda native 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 79. Machine pattern lambda native를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:thing "AI가 만든 설정"
 :check [:빠진칸? :중복칸? :몰래읽기?]
