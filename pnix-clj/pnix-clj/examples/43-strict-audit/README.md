...
기존 evaluator 결과를 바꾸지 않고, stricter Nix typing이면 어떤 source가 막힐지 audit-only로 분류하는 예제다.

## plain Clojure의 한계

Clojure truthiness는 `0`, 빈 collection, 문자열 등을 조건으로 허용한다. 그래서 plain eval은 Nix식 strict boolean frontier를 알려주지 않는다.

## pnix-clj 방식

`pnix-clj.strict-audit/report`는 corpus를 audit mode로 돌려 strict-ok/strict-violation/held를 분류하고, `strict-gate-report`는 strict-ok source만 opt-in strict mode로 재확인한다.

## 어디에 쓰나

typing/gate 정책을 강화하기 전에 실제 corpus에 어떤 영향이 있는지 behavior change 없이 측정할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 43. Strict audit 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 43. Strict audit를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
