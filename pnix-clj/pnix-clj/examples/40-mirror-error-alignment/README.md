# 40. Mirror error alignment

## 무엇을 보여주나

성공값만 비교하는 것이 아니라, 의도적으로 실패해야 하는 source도 evaluator/px-runtime/pnix-mirror가 같은 error frontier로 수렴해야 한다는 점을 보여준다.

## plain Clojure의 한계

plain `try/catch`는 exception 하나를 잡을 수 있지만, 여러 substrate의 error reason, phase, runtime receipt가 서로 맞는지 검증하지 않는다.

## pnix-clj 방식

`pnix-clj.mirror-error/report`는 negative fixture corpus를 돌리고 top/eval/runtime mirror error field가 기대값과 맞는지 alignment row로 남긴다.

## 어디에 쓰나

새 evaluator 분기나 runtime lowering을 추가할 때, 성공 케이스뿐 아니라 실패 semantics도 drift하지 않는지 확인하는 회귀 예제로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 40. Mirror error alignment 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 40. Mirror error alignment를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
