# 17-structural-search - structural search

plain text search는 `1 + 2`와 `3 + 4`가 같은 shape라는 사실도, 서로 같은 term이 아니라는 사실도 구조적으로 다루지 않는다.

pnix-clj의 `search`는 skeleton/free-vars/structural-distance로 후보를 만들고, equivalence는 `cas/structurally-equivalent?` 같은 confirmation에 맡긴다.

실행:

```sh
cd pnix-clj
clojure -M examples/17-structural-search/limit_clojure.clj
clojure -M examples/17-structural-search/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 17-structural-search - structural search 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 17-structural-search - structural search를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
