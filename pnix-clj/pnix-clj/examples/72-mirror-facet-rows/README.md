# 72. Mirror facet rows

## 무엇을 보여주나

`run-source` receipt 안의 clojure mirror, px runtime, pnix mirror, cross-mirror verdict row를 직접 읽는 예제다.

## plain Clojure의 한계

값 하나가 맞는지만 보면 host mirror와 px runtime, pnix mirror가 어디서 동의했는지 알 수 없다.

## pnix-clj 방식

`run-source`는 direct evaluator 결과뿐 아니라 mirror facet rows와 cross-mirror verdict를 포함한다.

## 어디에 쓰나

mirror path drift를 디버깅하거나, 어느 runtime facet에서 held/rejected가 났는지 보고 싶을 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 72. Mirror facet rows 문제를 어떻게 흉내 내는지 본다.
...
- plain 쪽 한계: plain Clojure 쪽은 두 함수를 비교할 수는 있지만, runtime별 error reason이나 collapse verdict를 일관된 schema로 남기지 않는다.
- pnix-clj 쪽 핵심: pnix-clj 쪽은 evaluator/runtime/mirror row를 분리하고, 같은 값뿐 아니라 같은 held frontier도 비교한다.
- 판단 기준: 서로 다른 runtime/lane이 같은 의미를 낸다는 사실을 한 번의 값 비교가 아니라 mirror row와 error alignment로 확인한다.
...
실무 흐름으로 바꾸면 이렇게 쓴다. 기존 runtime과 새 runtime을 병렬로 돌리고, value mismatch와 error mismatch를 다른 queue로 보낸다.
...
{:domain :runtime-migration
 :old-lane old-result
 :new-lane new-result
 :collapse? (= old-result new-result)
...
이전 설명: 서로 다른 runtime/lane이 같은 의미를 낸다는 사실을 한 번의 값 비교가 아니라 mirror row와 error alignment로 확인한다.

초딩 설명: 친구 두 명에게 같은 문제를 풀게 했을 때 둘 다 답이 같아야 한다. 한 명은 답을 냈고 한 명은 막혔다면, 어디서 막혔는지 같이 적는다.
