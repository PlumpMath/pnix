...
AI agent나 plugin system은 파일 읽기, 환경 변수, 네트워크 같은 host 권한을 쓰고 싶어 한다. 문제는 “이 호출이 허용된 권한인가?”와 “무슨 입력으로 어떤 결과를 냈는가?”가 로그에 남아야 한다는 점이다.

## plain Clojure의 한계

함수를 호출하면 host API가 바로 실행된다. 권한 확인을 함수마다 사람이 구현해야 하고, deny된 호출도 content-addressed witness로 남지 않는다.

## pnix-clj 방식

pnix source level에서는 `safe/safe-eval`의 `pure-only?` gate가 impure builtin을 실행 전 거부한다. Clojure/JVM interop level에서는 `interop/run-crossing`이 effect-class를 capability set과 비교하고 witness hash를 붙인다.

## 실제 응용

coding agent가 “파일 읽어도 됨?” 같은 tool call을 만들 때 사용한다. 기본은 `#{:pure}`로 막고, 사용자가 승인한 경우에만 `:file-read` 같은 effect-class를 grant한다.
...
(let [pnix-plugin-check (safe/safe-eval "builtins.readFile \"/etc/passwd\""
                                        {:pure-only? true})
      denied (interop/run-crossing :tool-read tool-meta input #{:pure} f)
      allowed (interop/run-crossing :tool-read tool-meta input #{:pure :file-read} f)]
...
비교하면, limit 파일은 plugin이 host 권한을 바로 쓴다. pnix-clj 파일은 source-level gate와 interop-level capability witness를 분리해 남긴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 87. Plugin capability boundary 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 87. Plugin capability boundary를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
