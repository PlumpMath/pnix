# 13-canonical-term-store - canonical term identity

plain Clojure의 `hash`나 `pr-str`는 term identity의 근거가 되기 어렵다. 같은 의미의 binder rename도 다른 문자열이 된다.

pnix-clj의 `cas`는 위치를 제거하고, let/lambda binder를 alpha-canonical form으로 바꾼 뒤 content address를 만든다. hash hit도 structural confirmation으로 확인한다.

실행:

```sh
cd pnix-clj
clojure -M examples/13-canonical-term-store/limit_clojure.clj
clojure -M examples/13-canonical-term-store/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 13-canonical-term-store - canonical term identity 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 13-canonical-term-store - canonical term identity를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
