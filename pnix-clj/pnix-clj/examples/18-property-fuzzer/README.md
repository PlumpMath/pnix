# 18-property-fuzzer - generative properties

plain Clojure에서 몇 개 sample을 실행하는 것은 counterexample search가 아니다.

pnix-clj의 `property-fuzzer`는 generated pnix source에 대해 cross-lane collapse, specializer soundness, cache soundness, proven arithmetic, machine agreement property를 검사하고 실패 시 shrink 가능한 counterexample을 남긴다. M7h 이후 report schema는 v4이고 `:machine-pass?`가 포함된다.

실행:

```sh
cd pnix-clj
clojure -M examples/18-property-fuzzer/limit_clojure.clj
clojure -M examples/18-property-fuzzer/pnix_clj_way.clj
```
...
(println "missing: generated pnix corpus, cross-lane collapse property, cache/specializer soundness property")
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 18-property-fuzzer - generative properties 문제를 어떻게 흉내 내는지 본다.
...
- pnix-clj 쪽 핵심: pnix-clj 쪽은 generated corpus, oracle row, proof result, cache result, machine agreement, strict held reason을 구조화해서 regression을 잡는다.
...
이전 설명: 몇 개의 happy path sample이 아니라, oracle·fuzzer·proof·machine agreement로 반례와 frontier를 찾는다.

초딩 설명: 시험 문제 두세 개만 맞았다고 전부 아는 게 아니다. 여러 문제를 자동으로 많이 내 보고, machine 친구와 evaluator 친구가 같은 답을 내는지도 같이 본다.
...
이전 설명: pnix-clj는 문제 출제기, 정답지, machine 채점표를 같이 둔다. 틀리면 `:reason`이나 smallest failing source로 어디서 막혔는지 남긴다.
...
```clojure
;; 18-property-fuzzer - generative properties를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:test-case generated-source
 :expected oracle-answer
 :actual actual-answer
 :pass? same?
 :machine-pass? machine-same?
