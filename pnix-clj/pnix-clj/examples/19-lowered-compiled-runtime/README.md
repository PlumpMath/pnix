# 19-lowered-compiled-runtime

plain Clojure는 eval과 compile 결과를 쉽게 비교하지 않는다.

pnix-clj 방식은 pnix source를 직접 evaluator로 실행한 값과, AST를 Clojure host form으로 lowering한 뒤 clj-meta compiler/evaluator로 실행한 값을 비교한다.
...
- lowering/lower-ast
- lowered host form
- clj-meta/eval-lowered
- compile receipt
- direct value와 compiled host value 일치 확인

이 예제는 pnix-clj가 단순 tree-walker만 가진 것이 아니라, clj-meta host proof lane을 통해 lowered compiled runtime 경로를 가진다는 것을 보여준다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 19-lowered-compiled-runtime 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 19-lowered-compiled-runtime를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
