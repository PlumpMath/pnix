...
CI나 agent loop에서 같은 의미의 코드를 formatting만 바꿔도 source-string cache는 miss가 난다. 그러면 느려지고, 더 나쁘게는 “이전 결과와 같은 의미인가?”를 cache가 설명하지 못한다.

## plain Clojure의 한계

문자열을 key로 memoization하면 `"1 + 2"`와 `" 1   +   2 "`는 다른 key다. 사람이 보기엔 같은 프로그램이어도 cache는 모른다.

## pnix-clj 방식

`cached-eval`은 parser AST에서 위치 정보를 제거한 canonical content hash와 epoch를 key로 쓴다. 그래서 공백/괄호 변형은 같은 content hash를 공유하고 hit 된다. impure/held/non-cacheable 값은 bypass한다.

## 실제 응용

formatter 적용 PR, codemod, AI refactor 후에 “의미가 같은 작은 식”이 cache 재사용되는지 확인한다. build cache, semantic CI, editor feedback loop에 연결하기 좋다.
...
```clojure
(defn string-keyed-eval [source f]
...
(assert (= :miss (:status (string-keyed-eval "1 + 2" (constantly 3)))))
(assert (= :miss (:status (string-keyed-eval " 1   +   2 " (constantly 3)))))
```
...
비교하면, limit 파일은 포맷 변경을 다른 key로 본다. pnix-clj 파일은 canonical AST content hash로 같은 의미의 refactor를 같은 cache entry로 본다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 88. Refactor cache stability 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 88. Refactor cache stability를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
