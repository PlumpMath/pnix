# 22-translation-validation - per-candidate validation

plain Clojure의 compile/run 성공은 그 compile 결과가 source 의미를 보존했다는 validation row가 아니다.

pnix-clj의 `translation-validation`은 parse, evaluator oracle, lowering, compile receipt, px-runtime, pnix mirror, cross-mirror를 validator catalog로 나눠 evidence를 남긴다.

실행:

```sh
cd pnix-clj
clojure -M examples/22-translation-validation/limit_clojure.clj
clojure -M examples/22-translation-validation/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 22-translation-validation - per-candidate validation 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 22-translation-validation - per-candidate validation를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
