# 36. Clojure projection report

## 무엇을 보여주나

plain Clojure 값과 host crossing을 pnix term으로 투영할 때, 단순 `eval` 결과가 아니라 fixture별 projection 검증 report가 필요하다는 점을 보여준다.

## plain Clojure의 한계

`eval`은 host 값을 만들 수 있지만, 그 값이 pnix 쪽에서 어떤 term으로 투영되는지, host crossing이 있었는지, projection runtime self-test가 통과했는지 남기지 않는다.

## pnix-clj 방식

`pnix-clj.clojure-projection/report`는 Clojure fixture corpus를 projection runtime에 통과시키고, accepted/held/rejected, runtime artifact, host crossing count를 하나의 report로 묶는다.

## 어디에 쓰나

Clojure macro/form/value를 pnix 쪽으로 넘기는 경계에서 "그냥 Clojure 값"이 아니라 검증된 projection receipt가 필요할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 36. Clojure projection report 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 36. Clojure projection report를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:dashboard "현재 기능 목록"
 :count total
 :hash report-hash
 :drift? changed?}
