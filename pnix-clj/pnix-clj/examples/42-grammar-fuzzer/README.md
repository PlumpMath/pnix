...
고정 fixture만이 아니라 seed가 있는 generated pnix source를 positive/error class로 만들어 `run-source` gate를 통과시키는 예제다.

## plain Clojure의 한계

랜덤 문자열을 만들고 `eval`하는 방식은 pnix grammar, expected status, lane summary, failure frontier를 함께 관리하지 않는다.

## pnix-clj 방식

`pnix-clj.grammar-fuzzer/report`는 deterministic seed로 positive/error source를 만들고, expected `:accepted`/`:held`와 실제 `run-source` 결과를 비교한다.

## 어디에 쓰나

parser/evaluator 변화 뒤에 손으로 고른 fixture 밖의 작은 문법 조합을 빠르게 흔들어 보는 smoke fuzzer로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 42. Grammar fuzzer 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 42. Grammar fuzzer를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
