# 56. px runtime run plan

## 무엇을 보여주나

vendored `.px` runtime resource graph를 실행 전에 boundary, entry parse, import edge plan으로 점검하는 예제다.

## plain Clojure의 한계

디렉터리에서 파일 목록을 읽는 것만으로는 entry source가 parse되는지, runtime root가 허용된 boundary인지, import graph가 끊기지 않았는지 알 수 없다.

## pnix-clj 방식

`pnix-clj.px-runtime/runtime-run-plan`은 runtime resource root, artifact count, import edges, boundary verdict, entry parse result를 구조화한다. 예제는 plan만 확인하고 runtime artifact execution은 하지 않는다.

## 어디에 쓰나

px runtime resource를 수정한 뒤, 파일이 빠졌거나 import edge가 깨졌는지 빠르게 점검할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 56. px runtime run plan 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 56. px runtime run plan를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
