# 44. Purity event spine

## 무엇을 보여주나

동일 source를 여러 번 재실행해 deterministic result hash를 확인하고, 그 사실을 snapshot에 묶인 event로 남기는 예제다.

## plain Clojure의 한계

같은 expression을 여러 번 eval해서 같은 값이 나와도, 그 결과가 어떤 runtime snapshot에 묶였는지, event log chain이 intact한지 남지 않는다.

## pnix-clj 방식

`pnix-clj.purity/purity-check!`는 source를 반복 실행하고 결과 hash가 같으면 `:purity/run` event를 append한다. divergence면 fail-closed로 `:purity/violation`을 남긴다.

## 어디에 쓰나

cache, witness, replay 전에 "이 결과가 재실행으로 확인된 deterministic evidence인가"를 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 44. Purity event spine 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 44. Purity event spine를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
