# 24-phase-separation — 한 덩어리 eval vs phase별 verdict

## 무엇을 보여주나

plain Clojure는 읽기(read), 평가(eval), host effect가 쉽게 한 덩어리로 섞인다.

pnix-clj는 같은 일을 다음 phase로 분리한다.

- parse
- static purity / capability check
- direct evaluator
- AST lowering
- lowered compiled host path
- capability/purity gate

## 왜 필요한가

메타서큘러 runtime에서 가장 위험한 것은 "실행되었다"와 "검증되었다"를 섞는 것이다.

pnix-clj는 값을 얻는 것과 phase verdict를 남기는 것을 분리한다.

pure source는 전체 phase를 통과한다.

impure source는 parse는 되지만, purity/capability phase에서 거부되고, pure-only evaluation은 held verdict가 된다.
...
pnix-clj 디렉터리에서:
    ...
    clojure -M examples/24-phase-separation/limit_clojure.clj
    clojure -M examples/24-phase-separation/pnix_clj_way.clj
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 24-phase-separation — 한 덩어리 eval vs phase별 verdict 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 24-phase-separation — 한 덩어리 eval vs phase별 verdict를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
