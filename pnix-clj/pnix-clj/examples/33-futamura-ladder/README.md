# 33-futamura-ladder — 1차/2차/3차 Futamura 상태를 정직하게 전시

## 무엇을 보여주나

03-specialization-futamura는 Futamura 1차 사영, 즉 특정 프로그램/정적입력에 대한 residual program을 보여준다.

33-futamura-ladder는 한 단계 위에서 전체 ladder 상태를 보여준다.

- 1st projection: built
- 2nd projection: built, cogen-free generating extension
- 3rd projection: built curried route, cogen-free and not self-application cogen
- Jones optimality: measured/structural witness, not mechanized proof

## 왜 필요한가

Futamura는 말만 하면 쉽게 과장된다.

pnix-clj는 다음을 구분해야 한다.

    만든 것
    구조적으로 증거를 가진 것
    cogen-free route로 만든 것
    self-application cogen으로 주장하지 않는 것

이 예제는 1차/2차는 실제 report 값으로 검증하고,
3차는 curried route로 전시하되 self-application cogen claim과 구분한다.

## 쉽게 말하면

plain Clojure:

    함수를 eval/compile할 수는 있다.
    하지만 interpreter, specializer, compiler, cogen의 관계를 증거로 남기지 않는다.

pnix-clj:

    direct interpreter 값
    1st projection 값
    2nd projection generating-extension 값
    3rd projection cogen-free curried route 상태

앞의 세 값이 같은지 비교한다.

그리고 2nd projection의 compiler-id가 프로그램마다 고정인지,
1st projection residual hash가 프로그램마다 달라지는지도 확인한다.

## 실행

pnix-clj 디렉터리에서:

    clojure -M examples/33-futamura-ladder/limit_clojure.clj
    clojure -M examples/33-futamura-ladder/pnix_clj_way.clj
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 33-futamura-ladder — 1차/2차/3차 Futamura 상태를 정직하게 전시 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 33-futamura-ladder — 1차/2차/3차 Futamura 상태를 정직하게 전시를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
