# 16-witness-replay - persisted witness replay

plain Clojure는 저장된 값과 새 값을 비교할 수는 있지만, 어떤 hash가 달라졌는지 구조화된 replay verdict를 주지 않는다.

pnix-clj의 `replay-witness`는 persisted source와 witness를 다시 실행해 term hash, result hash, evaluator, snapshot을 비교한다. 일치하면 `:reproduced`, 없거나 달라지면 별도 verdict가 나온다.

실행:

```sh
cd pnix-clj
clojure -M examples/16-witness-replay/limit_clojure.clj
clojure -M examples/16-witness-replay/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 16-witness-replay - persisted witness replay 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 16-witness-replay - persisted witness replay를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
