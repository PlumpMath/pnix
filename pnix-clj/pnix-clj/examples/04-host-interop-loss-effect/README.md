# 04-host-interop-loss-effect

plain Clojure는 host 값, eval, Java object, function value를 쉽게 섞을 수 있다.

문제는 crossing이 어떤 효과를 냈는지, 값 손실이 있었는지, 권한이 있었는지, 증거가 무엇인지가 기본적으로 남지 않는다는 점이다.

pnix-clj 방식은 pnix-clj.interop를 통해 Clojure/JVM <-> pnix crossing을 명시한다.

핵심 비교:

plain Clojure:
- 그냥 eval한다.
- Java object가 그냥 넘어간다.
- function value가 그냥 값처럼 섞인다.
- effect/loss/capability/witness가 기본으로 남지 않는다.

pnix-clj:
- direction을 남긴다.
- effect-class를 남긴다.
- loss-status를 남긴다.
- capability verdict를 남긴다.
- witness hash를 남긴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 04-host-interop-loss-effect 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 04-host-interop-loss-effect를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
