# 35-stage-tower-internals — self-hosting tower 내부 보기

## 무엇을 보여주나

11-self-hosting-convergence는 tower가 최종적으로 collapse 되는지를 보여준다.

35-stage-tower-internals는 그 안쪽을 보여준다.

pnix-clj tower는 하나의 source를 여러 phase/layer로 올린다.
...
그리고 adjacent pair evidence를 남긴다.
...
self-hosting convergence를 단순히 "성공했다"로만 보면 내부 증거 구조가 보이지 않는다.

pnix-clj 방식은 각 layer가 무엇을 했고, 어느 pair가 어떤 evidence로 동의했는지,
collapse witness가 어떤 hash와 cross-mirror verdict를 들고 있는지 보여준다.

## 11과 35의 차이

11-self-hosting-convergence:

    tower가 collapse 되는가?

35-stage-tower-internals:

    어떤 layer들이 있었는가?
    어떤 pair들이 동의했는가?
    collapse witness는 무엇인가?
    held frontier에서는 어느 layer가 막았는가?

## 실행

pnix-clj 디렉터리에서:

    clojure -M examples/35-stage-tower-internals/limit_clojure.clj
    clojure -M examples/35-stage-tower-internals/pnix_clj_way.clj
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 35-stage-tower-internals — self-hosting tower 내부 보기 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 35-stage-tower-internals — self-hosting tower 내부 보기를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
