# 47. Self-improve review queue

## 무엇을 보여주나

self-improve loop는 후보를 직접 적용하지 않고, witnessed/gated proposal을 ranked review queue로 만든다는 점을 보여준다.

## plain Clojure의 한계

함수 후보 목록을 점수화하거나 `eval`할 수는 있지만, 각 후보가 admitted witness를 얻었는지, 기본 정책에서 held/rejected 되었는지 event로 남기지 않는다.

## pnix-clj 방식

`pnix-clj.self-improve/evaluate-round`는 후보 batch를 self-mod gate에 태우고, default owner-hold 정책 아래 review queue와 round hash를 만든다.

## 어디에 쓰나

generator/synthesizer가 만든 후보를 곧장 적용하지 않고, 검증된 owner review queue로 넘기는 self-* loop body 예제로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 47. Self-improve review queue 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 47. Self-improve review queue를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
