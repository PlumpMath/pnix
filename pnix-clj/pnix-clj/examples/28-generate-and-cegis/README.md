# 28-generate-and-cegis - synthesis and refinement

plain Clojure에서 후보를 손으로 나열하면 search, dedup, counterexample refinement가 없다.

pnix-clj의 `generate`는 value-vector로 observationally equivalent 후보를 줄이고, `cegis`는 reference와 probe를 비교해 counterexample을 spec에 추가한다. 산술 fragment에서는 `arith-proof`로 `:proven`까지 올라간다.

실행:

```sh
cd pnix-clj
clojure -M examples/28-generate-and-cegis/limit_clojure.clj
clojure -M examples/28-generate-and-cegis/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 28-generate-and-cegis - synthesis and refinement 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 28-generate-and-cegis - synthesis and refinement를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
