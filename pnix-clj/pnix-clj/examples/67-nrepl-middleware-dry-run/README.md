# 67. nREPL middleware dry run

## 무엇을 보여주나

실제 nREPL 서버를 띄우지 않고도 `wrap-pnix-eval` middleware가 `eval` op를 pnix evaluator로 라우팅하는지 확인하는 예제다.

## plain Clojure의 한계

일반 nREPL eval은 Clojure code를 평가한다. pnix source를 editor에서 보내려면 eval op가 Clojure fallback이 아니라 pnix evaluator로 가야 한다.

## pnix-clj 방식

`pnix-clj.nrepl/wrap-pnix-eval`은 `op=eval`과 `code`가 있는 message를 `pnix-clj.core/eval-source`로 평가하고, transport에 rendered value와 done status를 보낸다.

## 어디에 쓰나

editor integration이나 AI tool session에서 pnix lane을 nREPL처럼 구동하되, Clojure eval과 언어 경계를 섞지 않도록 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 67. nREPL middleware dry run 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 67. nREPL middleware dry run를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:dashboard "현재 기능 목록"
 :count total
 :hash report-hash
 :drift? changed?}
