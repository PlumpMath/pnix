# 27-boolean-proof - boolean proof

boolean sample 몇 개가 맞아도 논리식이 전체 입력에서 같은지는 모른다.

pnix-clj의 `bool-proof`는 변수 수가 bounded인 boolean fragment를 exhaustive truth table로 검사해 `:proven` 또는 `:refuted`를 낸다.

실행:

```sh
cd pnix-clj
clojure -M examples/27-boolean-proof/limit_clojure.clj
clojure -M examples/27-boolean-proof/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 27-boolean-proof - boolean proof 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 27-boolean-proof - boolean proof를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
