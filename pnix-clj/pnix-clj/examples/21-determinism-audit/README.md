# 21-determinism-audit - repeated hash stability

plain Clojure에서 한 번 eval한 값은 runtime determinism의 증거가 아니다.

pnix-clj의 `determinism/report`는 source corpus를 반복 parse/eval하고 AST hash와 result hash가 안정적인지 확인한다.

실행:

```sh
cd pnix-clj
clojure -M examples/21-determinism-audit/limit_clojure.clj
clojure -M examples/21-determinism-audit/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 21-determinism-audit - repeated hash stability 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 21-determinism-audit - repeated hash stability를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
