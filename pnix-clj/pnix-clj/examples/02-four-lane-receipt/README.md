# 02-four-lane-receipt - 값 하나 vs 네 lane receipt

plain Clojure는 `(eval form)`으로 값은 얻지만, 그 값이 여러 독립 경로에서 같은 의미였는지 증거를 주지 않는다.

pnix-clj는 `run-source` 한 번으로 direct evaluator, lowered clj-meta bytecode, px-runtime, pnix mirror 값을 모으고 cross-mirror verdict를 남긴다.

실행:

```sh
cd pnix-clj
clojure -M examples/02-four-lane-receipt/limit_clojure.clj
clojure -M examples/02-four-lane-receipt/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 02-four-lane-receipt - 값 하나 vs 네 lane receipt 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 02-four-lane-receipt - 값 하나 vs 네 lane receipt를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
