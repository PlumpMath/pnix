# 70. Core compile pipeline

## 무엇을 보여주나

`pnix-clj.core/compile-source`가 parse, AST hash, lowering result, clj-meta compile receipt를 하나의 compile receipt로 묶는 예제다.

## plain Clojure의 한계

`eval`은 값을 얻지만 parse/lower/compile 증거와 source hash를 한 결과에 보존하지 않는다.

## pnix-clj 방식

`compile-source`는 source를 parse하고 lowered form으로 바꾼 뒤 clj-meta lane을 실행해 compile receipt를 반환한다.

## 어디에 쓰나

AI가 만든 source를 바로 실행하지 않고, 먼저 compile artifact와 evidence를 확인해야 할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 70. Core compile pipeline 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 70. Core compile pipeline를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
