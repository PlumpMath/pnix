# 39. Forward reference lift

## 무엇을 보여주나

pnix fixture corpus에서 forward reference가 어떤 lane에서는 lift되어 수렴하고, semantic error fixture는 일관되게 held 되는지 검증한다.

## plain Clojure의 한계

Clojure `let`은 순차 binding이라 뒤에서 정의되는 이름을 앞에서 바로 참조할 수 없다. 실패 여부는 exception으로 보이지만, 여러 lane의 forward-reference contract 검증은 없다.

## pnix-clj 방식

`pnix-clj.forward-reference/report`는 forward-ok fixture와 semantic-error fixture를 모두 돌려, evaluator/clj-meta/px-runtime lane의 status와 값을 비교한다.

## 어디에 쓰나

recursive let, mutually referenced fixture, lift된 binding semantics가 구현 변경 뒤에도 같은 방식으로 유지되는지 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 39. Forward reference lift 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 39. Forward reference lift를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
