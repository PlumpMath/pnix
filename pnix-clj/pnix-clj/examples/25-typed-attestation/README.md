# 25-typed-attestation — capability crossing의 typed witness

## 무엇을 보여주나

23-capability-gate는 host-effect 요구가 capability gate에서 held verdict가 되는 것을 보여줬다.

25-typed-attestation은 그 다음 단계다.

단순히 허용/거부만 보는 것이 아니라, boundary crossing에 다음 정보를 가진 typed witness를 붙인다.
...
권한 검사는 yes/no만으로 끝나면 약하다.

pnix-clj에서는 host crossing이 일어나거나 거부될 때도, 그 crossing이 어떤 종류였는지,
어떤 effect class였는지, 값이 lossless인지 opaque인지, input/output hash가 무엇인지 남긴다.

즉 capability verdict는 실행 허가이고, typed attestation은 그 허가/거부의 증거 봉투다.
...
pnix-clj:
    ...
    어떤 boundary crossing인가?
    어떤 capability가 필요한가?
    허용되었나?
    lossless인가 opaque인가?
    input/output witness hash는 무엇인가?
...
pnix-clj 디렉터리에서:
    ...
    clojure -M examples/25-typed-attestation/limit_clojure.clj
    clojure -M examples/25-typed-attestation/pnix_clj_way.clj
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 25-typed-attestation — capability crossing의 typed witness 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 25-typed-attestation — capability crossing의 typed witness를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
