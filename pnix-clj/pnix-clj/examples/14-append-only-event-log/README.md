# 14-append-only-event-log - event log and tamper evidence

plain Clojure의 atom/vector 로그는 과거 event를 바꿀 수 있고, Date/fn 같은 replay 불가능 payload도 쉽게 들어간다.

pnix-clj의 `store`는 pure EDN payload만 append하고, 각 event hash가 이전 hash에 연결되는 chain을 만든다. pointer 변경도 event로 기록된다.

실행:

```sh
cd pnix-clj
clojure -M examples/14-append-only-event-log/limit_clojure.clj
clojure -M examples/14-append-only-event-log/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 14-append-only-event-log - event log and tamper evidence 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 14-append-only-event-log - event log and tamper evidence를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
