...
pnix 값으로 쓰이는 EDN-ish data를 deterministic JSON string과 stable string hash로 다루는 예제다.

## plain Clojure의 한계

`hash`나 `pr-str`는 표현/키 순서에 기대기 쉽고, JSON object key ordering이나 value codec policy를 명시하지 않는다.

## pnix-clj 방식

`pnix-clj.json/write-json`은 map key를 정렬해 JSON을 쓰고, `read-json`은 string-keyed map/vector/nil/boolean/number로 되돌린다. `pnix-clj.hash/sha256`은 canonical JSON string을 SHA-256로 고정한다.

## 어디에 쓰나

report artifact, runtime mirror receipt, persisted evidence에서 값 identity를 JSON/해시로 비교해야 할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 62. Hash JSON codec 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 62. Hash JSON codec를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
