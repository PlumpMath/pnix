...
fixture corpus가 evaluator의 op, builtin, binary operator, branch를 실제로 얼마나 밟는지 측정하는 예제다.

## plain Clojure의 한계

`eval`을 여러 번 돌려도 어떤 evaluator branch가 실행됐는지 자동으로 남지 않는다. 테스트 통과와 coverage evidence는 다른 문제다.

## pnix-clj 방식

`pnix-clj.coverage/report`는 evaluator coverage atom을 바인딩해 fixture corpus를 실행하고, category별 covered/total/missing을 구조화한다.

## 어디에 쓰나

새 builtin이나 parser/evaluator branch를 추가한 뒤, corpus가 실제로 그 경로를 밟는지 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 41. Coverage report 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 41. Coverage report를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
