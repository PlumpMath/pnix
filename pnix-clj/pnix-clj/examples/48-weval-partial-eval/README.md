# 48. Weval partial eval

## 무엇을 보여주나

작은 interpreter/IR 경로에서 interpreted dispatch와 residual dispatch-free 실행을 비교하는 partial evaluation 예제다.

## plain Clojure의 한계

직접 만든 interpreter는 매 실행마다 tag dispatch를 한다. 특정 program에 특화된 residual function과 clj-meta agreement report를 자동으로 만들지 않는다.

## pnix-clj 방식

`pnix-clj.weval/report`는 corpus를 IR로 낮추고 residual function을 만들어, residual path가 dispatch-free이고 clj-meta lane과 같은 결과를 내는지 확인한다.

## 어디에 쓰나

Futamura/partial-eval 계열에서 "특화된 코드는 interpreter dispatch를 제거한다"는 주장을 작은 실행 report로 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 48. Weval partial eval 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 48. Weval partial eval를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
