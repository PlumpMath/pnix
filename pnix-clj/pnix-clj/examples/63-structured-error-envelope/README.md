# 63. Structured error envelope

## 무엇을 보여주나

실패를 단순 exception string이 아니라 phase/reason/schema가 있는 deterministic held envelope로 만드는 예제다.

## plain Clojure의 한계

`try/catch`로 exception을 잡으면 class/message 정도는 얻지만, pnix lane 전체에서 같은 schema로 읽을 수 있는 error envelope가 없다.

## pnix-clj 방식

`pnix-clj.error/held`와 `held-throwable`은 `:status :held`, top-level `:reason`, machine-readable `:error {:schema ... :phase ... :reason ...}`를 같이 만든다.

## 어디에 쓰나

parser/evaluator/lowering/runtime이 실패를 report와 mirror error corpus에서 같은 형태로 다루게 할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 63. Structured error envelope 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 63. Structured error envelope를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
