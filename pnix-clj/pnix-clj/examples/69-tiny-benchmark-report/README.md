# 69. Tiny benchmark report

## 무엇을 보여주나

성능 수치 자체를 고정하지 않고, 작은 source 하나로 benchmark report의 preflight, lane list, cache stats shape를 확인하는 예제다.

## plain Clojure의 한계

`time` macro는 wall-clock 출력만 남긴다. semantic preflight가 깨졌는지, parse/lower/full-report lane이 어떤 구조로 측정됐는지 machine-readable report를 만들지 않는다.

## pnix-clj 방식

`pnix-clj.benchmark/run-benchmark`는 먼저 `pnix/report` preflight를 통과시킨 뒤 parse/lower/full-report 측정 lane을 report로 반환한다. 예제는 source 하나와 iteration 1로 작게 제한한다.

## 어디에 쓰나

개발 중 local performance regression을 볼 때, timing 수치가 아니라 report shape와 semantic preflight를 먼저 확인하는 smoke benchmark로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 69. Tiny benchmark report 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 69. Tiny benchmark report를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:dashboard "현재 기능 목록"
 :count total
 :hash report-hash
 :drift? changed?}
