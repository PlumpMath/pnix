# 53. Rust-grounded slice

## 무엇을 보여주나

Rust pnix 구현에서 가져온 invariance corpus의 provenance, suite inventory, 첫 fixture cross-lane 검증을 보여준다.

## plain Clojure의 한계

소스 문자열 하나를 eval하면 어느 Rust test suite에서 왔는지, vendored source hash가 맞는지, imported test count가 맞는지 알 수 없다.

## pnix-clj 방식

`pnix-clj.rust-batch`는 manifest, suite source inventory, vendored `.px` fixture를 제공한다. 전체 batch report는 무거우므로 예제에서는 manifest/inventory와 첫 fixture `run-source`만 실행한다.

## 어디에 쓰나

cross-implementation corpus를 pnix-clj lane에 연결하되, 예제 실행 시간은 작은 slice로 유지하고 싶을 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 53. Rust-grounded slice 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 53. Rust-grounded slice를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
