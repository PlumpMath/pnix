# 09-clojure-form-fixture - Clojure form projection fixture

plain Clojure는 form을 읽고 실행할 수 있지만, host 값과 clj-meta 값, projection term이 같은 의미인지 고정하지 않는다.

pnix-clj의 `clojure-form` report는 fixture별 hash, host result, clj-meta compiled result, projection validation을 한 row로 묶는다.

실행:

```sh
cd pnix-clj
clojure -M examples/09-clojure-form-fixture/limit_clojure.clj
clojure -M examples/09-clojure-form-fixture/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 09-clojure-form-fixture - Clojure form projection fixture 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 09-clojure-form-fixture - Clojure form projection fixture를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
