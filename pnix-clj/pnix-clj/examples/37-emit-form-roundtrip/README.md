# 37. Emit form roundtrip

## 무엇을 보여주나

Clojure form을 analyzer AST로 낮춘 뒤 다시 form으로 emit했을 때, 원래 값과 emitted 값이 fresh namespace에서 같아야 한다는 roundtrip evidence를 보여준다.

## plain Clojure의 한계

`macroexpand`나 `eval`은 form이 실행되는지 확인할 수는 있지만, analyzer AST -> emitted form -> re-eval 경로가 같은 값을 내는지 fixture report로 고정하지 않는다.

## pnix-clj 방식

`pnix-clj.emit-form-roundtrip/report`가 literal, arithmetic, let, if, vector, host call fixture를 analyzer emit-form으로 왕복시키고 value equality를 검증한다.

## 어디에 쓰나

Clojure AST lowering/emit 경로를 pnix-clj 메타 경로의 증거 substrate로 사용할 때, emitter가 의미를 바꾸지 않았는지 확인하는 회귀 예제로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 37. Emit form roundtrip 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 37. Emit form roundtrip를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
