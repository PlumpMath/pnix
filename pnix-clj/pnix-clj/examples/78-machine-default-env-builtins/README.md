# 78. Machine default-env builtins

## 무엇을 보여주나

M7c 이후 abstract machine이 evaluator의 default env 아래에서 동작하고, `builtins.length`, bare `map`, `toString`, D2 strictness edge, capability-held builtin까지 evaluator와 같은 결과를 내는지 보여준다.

## plain Clojure의 한계

작은 interpreter를 만들면 기본 env와 builtin semantics를 직접 복제해야 한다. 복제한 builtin이 evaluator와 같은 strictness, partial application, held reason을 유지한다는 증거가 없다.

## pnix-clj 방식

`pnix-clj.machine/eval-source`는 machine control을 쓰되 builtin application은 evaluator public boundary를 공유한다. 예제는 evaluator와 machine의 comparable result가 같은지 직접 확인한다.

## 어디에 쓰나

machine fragment 확장, default-scope builtin 회귀, meta-circular control/value boundary를 확인할 때 쓴다.
...
비교하면, limit 파일은 builtin env가 없어서 바로 unbound가 된다. pnix-clj 파일은 evaluator와 machine을 같은 source로 실행해 default env/builtins 의미가 수렴하는지 검증한다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 78. Machine default-env builtins 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 78. Machine default-env builtins를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
