# 06-ast-lowering-roundtrip

plain Clojure는 source string을 eval하면 값은 얻을 수 있지만, source -> AST -> lowered host form -> compiled/evaluated host result 흐름을 하나의 evidence chain으로 보존하지 않는다.

pnix-clj 방식은 pnix source를 먼저 AST로 만들고, AST hash를 남긴 뒤, Clojure host form으로 lowering하고, clj-meta host proof lane으로 평가한다.
...
- eval-source result와 clj-meta result 비교
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 06-ast-lowering-roundtrip 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 06-ast-lowering-roundtrip를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
