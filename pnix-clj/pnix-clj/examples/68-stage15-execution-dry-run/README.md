# 68. Stage15 execution dry run

## 무엇을 보여주나

실제 clj-meta 명령을 실행하지 않고, `stage15/execute-plan`의 execution receipt shape를 fake runner로 확인하는 예제다.

## plain Clojure의 한계

명령을 실행했다고 가정한 map을 만들 수는 있지만, stage15 control plan hash, selected command ids, held count, receipt hash 같은 표준 execution report를 만들지 않는다.

## pnix-clj 방식

`pnix-clj.stage15/execute-plan`은 runner를 주입할 수 있다. 예제는 fake runner를 사용해 외부 프로세스 없이 `:stage15-execution-report`를 만든다.

## 어디에 쓰나

CI나 owner approval 흐름에서 실제 실행 전/후 report shape를 테스트하거나, stage15 command selection receipt를 검증할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 68. Stage15 execution dry run 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 68. Stage15 execution dry run를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
