...
무엇이 이미 구현되어 있는지 사람이 손으로 목록을 유지하지 않고, code-derived capability index로 확인하는 예제다.

## plain Clojure의 한계

README나 hand-written checklist는 구현이 늘거나 줄어도 자동으로 맞춰지지 않는다. report kind, deps alias, builtin surface, public API를 한 번에 대조하지 않는다.

## pnix-clj 방식

`pnix-clj.capabilities/index`는 evaluator builtins, report-artifact supported kinds, deps aliases, public API를 deterministic data로 렌더링한다.

## 어디에 쓰나

새 기능을 만들기 전에 이미 있는 report/API를 찾거나, capability docs drift를 확인하는 기반으로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 58. Capability index 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 58. Capability index를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
