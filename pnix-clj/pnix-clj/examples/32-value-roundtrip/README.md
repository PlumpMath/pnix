# 32-value-roundtrip - value projection closure

plain `pr-str`/`read-string`은 단순 Clojure data roundtrip일 뿐, pnix value projection의 증거가 아니다.

pnix-clj의 `value-roundtrip`은 pnix evaluator value를 canonical Clojure form으로 합성하고 clj-meta로 재평가해 같은 값으로 닫히는지 확인한다. function-like value는 `:held`로 둔다.

실행:

```sh
cd pnix-clj
clojure -M examples/32-value-roundtrip/limit_clojure.clj
clojure -M examples/32-value-roundtrip/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 32-value-roundtrip - value projection closure 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 32-value-roundtrip - value projection closure를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
