...
서비스 설정에는 필수 옵션과 선택 옵션이 있다. AI가 `port` 대신 `prt` 같은 typo를 만들면, 기본값이 조용히 들어가 장애로 이어질 수 있다.

## plain Clojure의 한계

Clojure destructuring은 기본값을 주기 쉽지만, typo와 extra key를 어떻게 처리할지 별도 contract가 필요하다. 기본값 fallback은 편하지만 설정 검토에서는 위험할 수 있다.

## pnix-clj 방식

Nix pattern lambda를 옵션 contract처럼 쓴다. `{ port, tls ? false }`는 `port`를 필수로 요구하고, `tls`는 default를 가진다. `...`를 붙이면 forward-compatible extra key를 허용한다. D19 semantics 덕분에 missing/extra/default가 real Nix 방식으로 평가된다.

## 실제 응용

서비스 템플릿, deployment module, generated config schema의 입구에서 사용한다. 엄격한 API면 `...`를 빼고, 확장 가능한 config면 `...`를 넣는다.
...
(defn service-port
  ...
  (if tls (+ port 443) port))

(assert (= 80 (service-port {:prt 8080})))
(assert (= 8080 (service-port {:port 8080 :debug true})))
```
...
비교하면, limit 파일은 typo를 default fallback으로 숨긴다. pnix-clj 파일은 strict contract와 ellipsis contract를 명시적으로 구분한다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 86. Service option contract 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 86. Service option contract를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:thing "AI가 만든 설정"
 :check [:빠진칸? :중복칸? :몰래읽기?]
