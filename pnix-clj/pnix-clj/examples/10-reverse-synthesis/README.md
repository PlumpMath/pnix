# 10-reverse-synthesis - Clojure form -> pnix source

plain Clojure는 form을 실행할 수는 있지만 그 form을 pnix source로 의미보존 투영하는 표준 절차가 없다.

pnix-clj의 `synthesize/form->pnix`는 작은 whitelist만 허용하고, 결과를 tower로 검증한다. Java interop 같은 host form은 추측하지 않고 `:held`로 남긴다.

실행:

```sh
cd pnix-clj
clojure -M examples/10-reverse-synthesis/limit_clojure.clj
clojure -M examples/10-reverse-synthesis/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 10-reverse-synthesis - Clojure form -> pnix source 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 10-reverse-synthesis - Clojure form -> pnix source를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
