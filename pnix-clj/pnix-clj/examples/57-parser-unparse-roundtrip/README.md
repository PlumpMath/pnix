# 57. Parser unparse roundtrip

## 무엇을 보여주나

pnix source를 AST로 parse하고, `unparse`로 다시 source를 만든 뒤 reparse했을 때 위치 metadata를 제외한 구조가 같아야 한다는 예제다.

## plain Clojure의 한계

`pr-str`/`read-string`은 Clojure data roundtrip이지 pnix grammar, precedence, attr binding, source metadata roundtrip이 아니다.

## pnix-clj 방식

`pnix-clj.parser/parse-source`, `pnix-clj.unparse/unparse`, `strip-positions`를 사용해 fully parenthesized pnix source를 만들고 structural equality를 확인한다.

## 어디에 쓰나

specializer residual source, code generator, report에 들어가는 pnix source rendering이 parser와 의미를 맞추는지 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 57. Parser unparse roundtrip 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 57. Parser unparse roundtrip를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
