# 54. Report artifact materialization

## 무엇을 보여주나

report namespace를 직접 호출하는 것을 넘어, report-artifact registry가 kind를 dispatch하고 versioned EDN artifact 파일로 materialize하는 과정을 보여준다.

## plain Clojure의 한계

맵을 `prn`하거나 `spit`하는 것은 파일을 만들 수는 있지만, supported kind registry, artifact version, content hash, bytes, deterministic report dispatch를 같이 보장하지 않는다.

## pnix-clj 방식

`pnix-clj.report-artifact/write-report!`는 supported kind를 report로 렌더링하고, EDN 파일을 쓰며 path/hash/bytes와 report body를 반환한다.

## 어디에 쓰나

CI artifact, docs artifact, drift debugging용 report를 사람이 열 수 있는 파일로 남겨야 할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 54. Report artifact materialization 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 54. Report artifact materialization를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:dashboard "현재 기능 목록"
 :count total
 :hash report-hash
 :drift? changed?}
