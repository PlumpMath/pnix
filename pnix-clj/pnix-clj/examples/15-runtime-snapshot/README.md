# 15-runtime-snapshot - runtime pinning

plain Clojure cache는 값이 어떤 evaluator/JVM/classpath에서 계산됐는지 보통 모른다.

pnix-clj의 `snapshot`은 evaluator version과 host symbol version을 content hash로 pin한다. 현재 runtime이 snapshot과 다르면 `resolve-under-snapshot`은 값을 내지 않고 `:held`로 닫힌다.

실행:

```sh
cd pnix-clj
clojure -M examples/15-runtime-snapshot/limit_clojure.clj
clojure -M examples/15-runtime-snapshot/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 15-runtime-snapshot - runtime pinning 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 15-runtime-snapshot - runtime pinning를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
