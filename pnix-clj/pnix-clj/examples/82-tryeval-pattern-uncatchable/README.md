# 82. tryEval pattern uncatchable

## 무엇을 보여주나

D19 pattern application error가 `builtins.tryEval`에 잡히지 않는다는 경계를 보여준다. `throw`와 `assert false`는 `success = false`로 catch되지만, extra key나 non-attrset pattern argument는 held reason으로 바깥까지 나온다.

## plain Clojure의 한계

`try/catch Throwable`은 직접 던진 예외와 type/pattern error를 같은 방식으로 잡는다. Nix의 catchable/uncatchable taxonomy를 기본 제공하지 않는다.

## pnix-clj 방식

`pnix/eval-source`가 D3/D19 taxonomy를 공유해 `throw`/`assert`와 pattern application type error를 구분한다. pattern error는 `tryEval` 내부에 있어도 `:held`로 남는다.

## 어디에 쓰나

AI가 생성한 function application을 `tryEval`로 감싸도 안전하게 복구될 수 있는지, 아니면 type error frontier로 중단해야 하는지 판별할 때 쓴다.
...
(assert (= false (:success (try-eval #(throw (ex-info "throw" {}))))))
(assert (= false (:success (try-eval #(throw (ex-info "pattern type error" {}))))))
```
...
비교하면, limit 파일은 모든 Throwable을 catch 결과로 바꾼다. pnix-clj 파일은 real Nix taxonomy에 맞춰 catchable throw/assert와 uncatchable pattern error를 분리한다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 82. tryEval pattern uncatchable 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 82. tryEval pattern uncatchable를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:thing "AI가 만든 설정"
 :check [:빠진칸? :중복칸? :몰래읽기?]
