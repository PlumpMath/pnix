# 34-classfile-trust-receipt - JVM artifact and trust boundary

plain eval은 값이 나왔다는 사실만 보여준다. JVM bytecode artifact가 어떤 hash를 갖는지, verifier가 통과했는지, cross-lane voting의 common-mode risk는 무엇인지 남기지 않는다.

pnix-clj의 `classfile-receipt`는 clj-meta가 만든 classfile artifact를 hash와 verification row로 요약하고, `trust` report는 claim boundary와 residual risk를 명시한다.

실행:

```sh
cd pnix-clj
clojure -M examples/34-classfile-trust-receipt/limit_clojure.clj
clojure -M examples/34-classfile-trust-receipt/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 34-classfile-trust-receipt - JVM artifact and trust boundary 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 34-classfile-trust-receipt - JVM artifact and trust boundary를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
