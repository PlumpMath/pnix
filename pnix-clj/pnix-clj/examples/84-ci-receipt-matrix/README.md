...
CI에서 “테스트 통과”만 보면 어떤 실행 경로가 맞았는지 모른다. 특히 AI가 코드를 바꾼 경우, direct evaluator는 맞지만 lowered path나 runtime path가 깨지는 식의 문제가 생길 수 있다.

## plain Clojure의 한계

`eval`을 돌려 pass/fail을 만들 수는 있다. 하지만 실패가 어떤 의미론적 reason인지, 여러 substrate가 같은 값으로 수렴했는지, CI artifact로 남길 receipt가 무엇인지는 직접 설계해야 한다.

## pnix-clj 방식

정상 케이스는 `pnix/run-source`로 evaluator/clj-meta/px-runtime receipt를 확인한다. 실패 케이스는 `pnix/eval-source`의 held reason을 고정한다. 이렇게 만들면 CI row 하나가 값, lane 존재, verdict, reason을 함께 가진다.

## 실제 응용

PR마다 “작은 의미론 smoke suite”를 돌릴 수 있다. formatting refactor, lowering 변경, runtime 변경이 들어오면 단순 pass/fail보다 `:all-lanes-agree`가 깨지는지 보는 쪽이 더 빠르게 원인을 좁힌다.
...
비교하면, limit 파일은 pass/fail만 남긴다. pnix-clj 파일은 CI row에 lane receipt와 held reason을 같이 남긴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 84. CI receipt matrix 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 84. CI receipt matrix를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
