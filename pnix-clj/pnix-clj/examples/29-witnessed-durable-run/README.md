# 29-witnessed-durable-run - integrated witnessed run

plain 저장은 값과 source를 기록할 수는 있지만, 그 실행이 어떤 runtime snapshot과 evidence event로 admission됐는지 묶지 않는다.

pnix-clj의 `run-witnessed-durable`은 term hash, snapshot, tower collapse, mirror-chain, purity rerun, witness admission, persistent event log를 한 결과로 묶는다.

실행:

```sh
cd pnix-clj
clojure -M examples/29-witnessed-durable-run/limit_clojure.clj
clojure -M examples/29-witnessed-durable-run/pnix_clj_way.clj
```
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 29-witnessed-durable-run - integrated witnessed run 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 29-witnessed-durable-run - integrated witnessed run를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
