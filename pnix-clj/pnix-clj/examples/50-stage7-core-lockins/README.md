# 50. Stage7 core lockins

## 무엇을 보여주나

self-hosting core fixture가 stage7 lockin corpus로 고정되어 있고, 현재 evaluator/tower path에서 모두 accepted 되는지 검증한다.

## plain Clojure의 한계

plain `load-string`은 Clojure 코드를 실행할 뿐, pnix stage7 core fixture lineage, oracle result, lane receipt를 함께 검증하지 않는다.

## pnix-clj 방식

`pnix-clj.stage7-core/report`는 committed lockin fixture를 `pnix/report`에 통과시키고 fixture hash, lineage, accepted/held/rejected를 보존한다.

## 어디에 쓰나

self-hosting core나 runtime fixture를 손댄 뒤, lockin corpus가 여전히 현재 구현에서 닫혀 있는지 확인하는 regression 예제로 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 50. Stage7 core lockins 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 50. Stage7 core lockins를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
