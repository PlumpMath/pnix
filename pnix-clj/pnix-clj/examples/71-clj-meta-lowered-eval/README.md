# 71. clj-meta lowered eval

## 무엇을 보여주나

pnix source를 lowered Clojure form으로 바꾼 뒤, clj-meta compile/eval lane에서 값과 compile receipt를 얻는 예제다.

## plain Clojure의 한계

`eval`은 Clojure form을 실행할 수 있지만 clj-meta compiler/evaluator의 determinism receipt와 API agreement를 남기지 않는다.

## pnix-clj 방식

`core/lower-source`로 lowered form을 얻고, `clj-meta/eval-lowered`가 clj-meta compile/eval boundary를 witness attached result로 반환한다.

## 어디에 쓰나

pnix source가 JVM/clj-meta host proof lane에서 같은 값으로 평가되는지 직접 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 71. clj-meta lowered eval 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 71. clj-meta lowered eval를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:program source
