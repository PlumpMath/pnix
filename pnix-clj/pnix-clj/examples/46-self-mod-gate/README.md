...
자체 수정 제안이 witness를 얻더라도 기본 정책에서는 자동 promote되지 않고 owner authorization 전까지 held 되는 불변식을 보여준다.

## plain Clojure의 한계

`alter-var-root`, `load-string`, atom swap 같은 방식은 런타임 값을 바로 바꿀 수 있지만, proposed/held/admitted event trail과 owner gate가 없다.

## pnix-clj 방식

`pnix-clj.self-mod-gate/propose-and-gate`는 proposed source를 witnessed-run spine에 태운 뒤, 기본 `:owner-hold` 정책에서 `:held` decision을 event로 남긴다.

## 어디에 쓰나

self-* loop가 "수정 제안"과 "실제 적용"을 섞지 않도록, no-auto-promotion boundary를 회귀 예제로 고정할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 46. Self-mod gate 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 46. Self-mod gate를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
