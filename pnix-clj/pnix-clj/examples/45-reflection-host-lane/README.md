# 45. Reflection host lane

## 무엇을 보여주나

JVM/Clojure/classpath/namespace/Var metadata 같은 host-varying 입력을 stable EDN snapshot과 host-lane-id로 고정하는 예제다.

## plain Clojure의 한계

`all-ns`, `ns-publics`, Var 객체, classpath 문자열을 직접 읽을 수는 있지만, 정렬된 pure EDN snapshot과 hash identity로 pin하지 않는다.

## pnix-clj 방식

`pnix-clj.reflect`는 namespace, Var metadata, classpath, JVM version을 stable snapshot으로 만들고 같은 process 안에서 deterministic hash를 제공한다.

## 어디에 쓰나

compile/cache/witness 결과가 어떤 JVM host lane에서 나온 것인지 재현 가능하게 묶어야 할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 45. Reflection host lane 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 45. Reflection host lane를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
