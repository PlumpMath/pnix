# 08-clojure-reader-or-edn-embed-pnix — EDN reader data 안의 pnix source 검증

## 무엇을 보여주나

plain Clojure/EDN reader는 tagged literal을 읽을 수 있다. 하지만 그 tag가 담은 pnix source를
parse/purity/tower/witness로 검증하지는 않는다.

pnix-clj 방식은 Clojure reader 자체를 바꾸지 않는다.

대신 EDN tagged literal을 안전한 데이터로 읽은 뒤, 그 안의 pnix source를 pnix-clj의 검증 경로로 보낸다.

흐름은 이렇다.

    EDN read
    → #px tagged literal을 데이터로 수집
    → pnix parse
    → static purity / capability check
    → tower/run-tower
    → collapse witness

## 왜 필요한가

pnix-hy에는 Hy reader macro / #px embed 같은 방향이 있다.

pnix-clj에서는 Hy reader macro를 복제하지 않는다. Clojure/JVM 쪽 대응물은 다음이다.

    Clojure reader / EDN tagged literal / data-first embed
    → pnix source
    → pnix-clj tower verification

즉 reader 단계에서 실행하지 않는다. reader는 데이터만 만든다. 실행/검증은 pnix-clj gate/tower가 맡는다.
...
pnix-clj:
    ...
    그 데이터를 읽은 뒤,
    source가 순수한지 보고,
    tower에서 42로 collapse 되는지 확인하고,
    source/AST witness를 남긴다.

## 실행

pnix-clj 디렉터리에서:

    clojure -M examples/08-clojure-reader-or-edn-embed-pnix/limit_clojure.clj
    clojure -M examples/08-clojure-reader-or-edn-embed-pnix/pnix_clj_way.clj
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 08-clojure-reader-or-edn-embed-pnix — EDN reader data 안의 pnix source 검증 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 08-clojure-reader-or-edn-embed-pnix — EDN reader data 안의 pnix source 검증를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
