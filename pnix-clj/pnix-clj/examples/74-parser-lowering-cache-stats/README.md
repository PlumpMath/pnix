# 74. Parser lowering cache stats

## 무엇을 보여주나

parser cache와 lowering cache가 hit/miss/entry stats를 노출하고, 같은 source/AST 반복 처리에서 cache evidence를 남기는 예제다.

## plain Clojure의 한계

`read-string`이나 직접 transform은 cache hit/miss를 표준 구조로 보여주지 않는다.

## pnix-clj 방식

`parser/clear-parse-cache!`, `parse-cache-stats`, `lowering/clear-lower-cache!`, `lower-cache-stats`가 parse/lower 단계의 constructive trace를 확인하게 한다.

## 어디에 쓰나

AI가 같은 후보를 반복 생성하거나 report가 corpus를 반복 도는 상황에서 parse/lower cache가 실제로 작동하는지 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 74. Parser lowering cache stats 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 74. Parser lowering cache stats를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
