# 75. Clojure projection host crossing

## 무엇을 보여주나

`pnix-clj.clojure-projection.host`가 Clojure reader/eval host crossing을 직접 실행하되, interop metadata와 witness를 붙이는 예제다.

## plain Clojure의 한계

`read-string`과 `eval`은 host 값을 만들지만, 그 값이 projection fixture lane의 어떤 effect crossing인지, capability가 허용됐는지, input/output hash witness가 무엇인지 남기지 않는다.

## pnix-clj 방식

`read-host-value`는 `*read-eval* false`로 reader 값을 만들고, `host-eval-source`는 host eval crossing을 `interop/run-crossing`에 태워 `:interop`, `:capability`, `:witness`를 붙인다.

## 어디에 쓰나

Clojure projection fixture를 디버깅하거나, host value가 pnix projection으로 넘어오기 전 crossing evidence를 확인할 때 쓴다.
...
비교하면, limit 파일은 host reader/eval 값을 바로 만든다. pnix-clj 파일은 같은 host crossing을 projection host API에 태우고 `:status`, interop metadata, capability, witness hash를 확인한다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 75. Clojure projection host crossing 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 75. Clojure projection host crossing를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:request "파일 읽기"
 :allowed? false
 :status :held
