# 30-verifying-cache — memoize vs verified cache reuse

## 무엇을 보여주나

plain Clojure의 memoize는 함수 인자 기준 캐시다. 같은 의미라도 문자열 표현이 다르면 다른 key가 된다.
또 cache hit가 fresh evaluation과 같은지, purity/capability 조건을 통과했는지, 어떤 content key로 재사용됐는지에 대한
pnix식 verification receipt를 기본으로 주지 않는다.

pnix-clj의 verifying cache 예제는 cache hit를 그냥 믿지 않는다.

- content-addressed key 확인
- miss/hit 상태 확인
- fresh evaluator 값과 cached value 대조
- static purity verdict 확인
- impure source는 cache가 아니라 bypass 되는지 확인

## 왜 필요한가

캐시는 성능 최적화지만, pnix-clj에서는 semantic authority가 아니다.
...
가 아니라:
    ...
    cache hit가 났다
    fresh/direct evaluator와 같은 값인가?
    purity gate를 통과한 source인가?
    content-address key가 같은가?
    bypass되어야 할 것은 bypass되었는가?
...
12-content-addressed-cache:
    ...
    캐시가 source string이 아니라 정본 content address를 쓴다는 것을 보여준다.

30-verifying-cache:

    그 cache reuse가 fresh eval / purity verdict / cache key와 대조되어
    검증 가능한 재사용인지 보여준다.
...
pnix-clj 디렉터리에서:
    ...
    clojure -M examples/30-verifying-cache/limit_clojure.clj
    clojure -M examples/30-verifying-cache/pnix_clj_way.clj
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 30-verifying-cache — memoize vs verified cache reuse 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 30-verifying-cache — memoize vs verified cache reuse를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:answer 42
 :receipt "어떤 길로 계산했는지"
 :hash "답의 지문"
 :replay "나중에 다시 확인 가능"}
