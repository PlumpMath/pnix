# 51. Import module fixture

## 무엇을 보여주나

파일시스템 import를 직접 열지 않고, 명시적인 target string -> pnix source map으로 import boundary를 검증하는 예제다.

## plain Clojure의 한계

Clojure `load-file`이나 `slurp`는 실제 파일 경로에 의존한다. 어떤 module source가 import target에 대응했는지, 그 대응이 evaluator/lowering/runtime lane 모두에 전달됐는지 receipt로 남기지 않는다.

## pnix-clj 방식

`pnix-clj.import-module/cases`는 in-memory import fixture를 제공하고, `run-source`는 `:import-modules` map을 모든 lane에 전달해 `import ./m`이 같은 값으로 수렴하는지 확인한다.

## 어디에 쓰나

import semantics를 파일 IO 없이 fixture화하거나, import lowering이 host evaluator와 같은 module map을 쓰는지 확인할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 51. Import module fixture 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 51. Import module fixture를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:input source
 :step :validate
 :status :ok
 :reason nil
