...
pnix REPL이 evaluator result를 사람이 읽는 문자열로 렌더링하고, `eval-print`가 pnix source를 평가해 출력하는 경계를 보여준다.

## plain Clojure의 한계

`pr-str`는 Clojure 값 표현일 뿐 pnix value rendering policy가 아니다. closure/builtin/attrset/list를 pnix REPL 형식으로 보여주지 않는다.

## pnix-clj 방식

`pnix-clj.repl/render`는 pnix value를 compact하게 렌더링하고, `eval-print`는 `pnix-clj.core/eval-source`를 통해 source를 평가해 값 또는 error를 출력한다.

## 어디에 쓰나

CLI, socket REPL, nREPL middleware가 사람이 볼 출력 형식을 공유해야 할 때 쓴다.
...
```clojure
;; limit_clojure.clj
;; - plain Clojure에서 66. REPL rendering 문제를 어떻게 흉내 내는지 본다.
...
```clojure
;; 66. REPL rendering를 실제 서비스에 붙이면 이런 모양의 기록을 남긴다.
{:dashboard "현재 기능 목록"
 :count total
 :hash report-hash
 :drift? changed?}
