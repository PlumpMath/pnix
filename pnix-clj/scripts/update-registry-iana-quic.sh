#!/usr/bin/env bash
# IANA QUIC Registries XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-quic"
URL="https://www.iana.org/assignments/quic/quic.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/quic.xml"
SHA=$(shasum -a 256 "$OUT_DIR/quic.xml" | awk '{print $1}')
