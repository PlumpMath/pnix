#!/usr/bin/env bash
# IANA Well-Known URIs XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-well-known-uris"
URL="https://www.iana.org/assignments/well-known-uris/well-known-uris.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/well-known-uris.xml"
SHA=$(shasum -a 256 "$OUT_DIR/well-known-uris.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-well-known-uris",
  "project":"IANA Well-Known URIs Registry",
  ...
  "source_url":url,
  "source_path":"well-known-uris.xml",
  "source_sha256":sha
