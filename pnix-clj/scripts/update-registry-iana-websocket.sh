#!/usr/bin/env bash
# IANA WebSocket Registries XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-websocket"
URL="https://www.iana.org/assignments/websocket/websocket.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/websocket.xml"
SHA=$(shasum -a 256 "$OUT_DIR/websocket.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-websocket",
  "project":"IANA WebSocket Registries",
