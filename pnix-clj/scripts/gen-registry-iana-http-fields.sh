#!/usr/bin/env bash
# IANA HTTP Field Name Registry XML -> pnix attrset source. redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-http-fields"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-http-fields.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; status = "%s"; xrefs = %s; }' % (esc(child_text(rec,'value')), esc(child_text(rec,'status')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-http-fields.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-http-fields.sh')
lines.append('{')
lines.append('  schema = "registry.iana.http_fields.v1";')
lines.append('  source = {')
...
lines.append('  };')
lines.append('  registry = { id = "field-names"; title = "Hypertext Transfer Protocol (HTTP) Field Name Registry"; records = [')
for rec in records:
    lines.append('    %s' % record_literal(rec))
lines.append('  ]; };')
lines.append('  registry_count = 1;')
lines.append('  record_count = %d;' % len(records))
lines.append('  generated_note = "IANA HTTP Field Name XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
