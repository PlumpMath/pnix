#!/usr/bin/env bash
# IANA Link Relation Types XML -> pnix attrset source. 기관 XML registry columns 보존, redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-link-relations"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-link-relations.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    return '{ value = "%s"; description = "%s"; spec = "%s"; }' % (
      esc(child_text(rec,'value')), esc(child_text(rec,'description')), esc(child_text(rec,'spec')))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-link-relations.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-link-relations.sh')
lines.append('{')
lines.append('  schema = "registry.iana.link_relations.v1";')
lines.append('  source = {')
...
lines.append('  };')
lines.append('  registry = { id = "link-relations-1"; title = "Link Relation Types"; records = [')
for rec in records:
    lines.append('    %s' % record_literal(rec))
lines.append('  ]; };')
lines.append('  registry_count = 1;')
lines.append('  record_count = %d;' % len(records))
lines.append('  generated_note = "IANA Link Relation Types XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
