#!/usr/bin/env bash
# IANA CoRE Parameters XML -> pnix attrset source. redb 적재까지만, graph/mirror/math 연결 없음.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-core-parameters"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-core-parameters.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def lit_list(xs): return '[ ' + ' '.join('"%s"' % esc(x) for x in xs if x) + ' ]'
def xref_text(x):
    vals=[]
...
def record_literal(rec):
    ...
    return '{ fields = [ %s ]; xrefs = %s; }' % (' '.join(fields), lit_list(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-core-parameters.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-core-parameters.sh')
lines.append('{')
lines.append('  schema = "registry.iana.core_parameters.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA CoRE Parameters XML structural transcription only; XML child tag names preserved as field name/value pairs; no graph/mirror/math wiring";')
lines.append('}')
