#!/usr/bin/env bash
# IANA IPv6 Parameters XML 최신 snapshot 수집. redb 적재는 하지 않는다.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-ipv6-parameters"
URL="https://www.iana.org/assignments/ipv6-parameters/ipv6-parameters.xml"
mkdir -p "$SRC_DIR"
TMP="$SRC_DIR/ipv6-parameters.xml.tmp"
OUT="$SRC_DIR/ipv6-parameters.xml"
curl -fsSL "$URL" -o "$TMP"
mv "$TMP" "$OUT"
...
cat > "$SRC_DIR/manifest.json" <<JSON
{
  ...
  "source_id": "iana-ipv6-parameters",
  "project": "IANA IPv6 Parameters",
  ...
  "source_url": "$URL",
  "source_path": "ipv6-parameters.xml",
  "source_sha256": "$SHA",
  "source_bytes": $BYTES
}
JSON
