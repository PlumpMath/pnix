#!/usr/bin/env bash
# IANA DNS Parameters XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-dns-parameters"
URL="https://www.iana.org/assignments/dns-parameters/dns-parameters.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/dns-parameters.xml"
SHA=$(shasum -a 256 "$OUT_DIR/dns-parameters.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-dns-parameters",
  "project":"IANA DNS Parameters Registry",
  ...
  "source_url":url,
  "source_path":"dns-parameters.xml",
  "source_sha256":sha
