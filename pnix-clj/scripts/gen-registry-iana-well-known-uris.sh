#!/usr/bin/env bash
# IANA Well-Known URIs XML -> pnix attrset source. redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-well-known-uris"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-well-known-uris.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; controller = "%s"; status = "%s"; related = "%s"; xrefs = %s; }' % (
      esc(child_text(rec,'value')), esc(child_text(rec,'controller')), esc(child_text(rec,'status')), esc(child_text(rec,'related')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-well-known-uris.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-well-known-uris.sh')
lines.append('{')
lines.append('  schema = "registry.iana.well_known_uris.v1";')
lines.append('  source = {')
...
lines.append('  };')
lines.append('  registry = { id = "well-known-uris-1"; title = "Well-Known URIs"; records = [')
for rec in records:
    lines.append('    %s' % record_literal(rec))
lines.append('  ]; };')
lines.append('  registry_count = 1;')
lines.append('  record_count = %d;' % len(records))
lines.append('  generated_note = "IANA Well-Known URIs XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
