#!/usr/bin/env bash
# IANA ISAKMP Registry XML 최신 snapshot 수집. redb 적재는 하지 않는다.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-isakmp-registry"
URL="https://www.iana.org/assignments/isakmp-registry/isakmp-registry.xml"
mkdir -p "$SRC_DIR"
TMP="$SRC_DIR/isakmp-registry.xml.tmp"
OUT="$SRC_DIR/isakmp-registry.xml"
curl -fsSL "$URL" -o "$TMP"
mv "$TMP" "$OUT"
...
cat > "$SRC_DIR/manifest.json" <<JSON
{
  ...
  "source_id": "iana-isakmp-registry",
  "project": "IANA ISAKMP Registry",
  ...
  "source_url": "$URL",
  "source_path": "isakmp-registry.xml",
  "source_sha256": "$SHA",
  "source_bytes": $BYTES
}
JSON
