#!/usr/bin/env bash
# IANA IPv4 Multicast Address Space XML 최신 snapshot 수집. redb 적재는 하지 않는다.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-multicast-addresses"
URL="https://www.iana.org/assignments/multicast-addresses/multicast-addresses.xml"
mkdir -p "$SRC_DIR"
TMP="$SRC_DIR/multicast-addresses.xml.tmp"
OUT="$SRC_DIR/multicast-addresses.xml"
curl -fsSL "$URL" -o "$TMP"
mv "$TMP" "$OUT"
...
cat > "$SRC_DIR/manifest.json" <<JSON
{
  ...
  "source_id": "iana-multicast-addresses",
  "project": "IANA IPv4 Multicast Address Space",
  ...
  "source_url": "$URL",
  "source_path": "multicast-addresses.xml",
  "source_sha256": "$SHA",
  "source_bytes": $BYTES
}
JSON
