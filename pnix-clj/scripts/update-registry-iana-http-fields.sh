#!/usr/bin/env bash
# IANA HTTP Field Name Registry XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-http-fields"
URL="https://www.iana.org/assignments/http-fields/http-fields.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/http-fields.xml"
SHA=$(shasum -a 256 "$OUT_DIR/http-fields.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-http-fields",
  "project":"IANA HTTP Field Name Registry",
