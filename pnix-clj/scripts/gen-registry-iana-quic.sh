#!/usr/bin/env bash
# IANA QUIC Registries XML -> pnix attrset source. redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-quic"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-quic.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-quic.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-quic.sh')
lines.append('{')
lines.append('  schema = "registry.iana.quic.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA QUIC Registries XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
