#!/usr/bin/env bash
# IANA Protocol Numbers XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-protocol-numbers"
URL="https://www.iana.org/assignments/protocol-numbers/protocol-numbers.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/protocol-numbers.xml"
SHA=$(shasum -a 256 "$OUT_DIR/protocol-numbers.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-protocol-numbers",
  "project":"IANA Protocol Numbers Registry",
  ...
  "source_url":url,
  "source_path":"protocol-numbers.xml",
  "source_sha256":sha
