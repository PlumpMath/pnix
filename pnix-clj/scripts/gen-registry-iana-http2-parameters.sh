#!/usr/bin/env bash
# IANA HTTP/2 Parameters XML -> pnix attrset source. redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-http2-parameters"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-http2-parameters.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; name = "%s"; description = "%s"; initial = "%s"; xrefs = %s; }' % (
      esc(child_text(rec,'value')), esc(child_text(rec,'name')), esc(child_text(rec,'description')), esc(child_text(rec,'initial')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-http2-parameters.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-http2-parameters.sh')
lines.append('{')
lines.append('  schema = "registry.iana.http2_parameters.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA HTTP/2 Parameters XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
