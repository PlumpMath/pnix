#!/usr/bin/env bash
# IANA IPv6 Multicast Address Space XML 최신 snapshot 수집. redb 적재는 하지 않는다.
set -euo pipefail
...
URL="https://www.iana.org/assignments/ipv6-multicast-addresses/ipv6-multicast-addresses.xml"
mkdir -p "$SRC_DIR"
TMP="$SRC_DIR/ipv6-multicast-addresses.xml.tmp"
OUT="$SRC_DIR/ipv6-multicast-addresses.xml"
curl -fsSL "$URL" -o "$TMP"
mv "$TMP" "$OUT"
...
cat > "$SRC_DIR/manifest.json" <<JSON
{
  ...
  "source_id": "iana-ipv6-multicast-addresses",
  "project": "IANA IPv6 Multicast Address Space",
  ...
  "source_url": "$URL",
  "source_path": "ipv6-multicast-addresses.xml",
  "source_sha256": "$SHA",
  "source_bytes": $BYTES
}
JSON
