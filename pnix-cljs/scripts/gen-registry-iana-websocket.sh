#!/usr/bin/env bash
# IANA WebSocket Registries XML -> pnix attrset source. redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-websocket"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-websocket.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ id = "%s"; value = "%s"; common = "%s"; status = "%s"; description = "%s"; contact = "%s"; incompatible = "%s"; xrefs = %s; }' % (
      esc(child_text(rec,'id')), esc(child_text(rec,'value')), esc(child_text(rec,'common')), esc(child_text(rec,'status')), esc(child_text(rec,'description')), esc(child_text(rec,'contact')), esc(child_text(rec,'incompatible')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-websocket.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-websocket.sh')
lines.append('{')
lines.append('  schema = "registry.iana.websocket.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA WebSocket Registries XML structural transcription only; definition prose excluded; no graph/mirror/math wiring";')
lines.append('}')
