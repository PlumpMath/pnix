...
PLAN="$ROOT/stdlib/lib/corpus/unicode-cldr-store-plan.px"
CHUNK_DIR="$ROOT/stdlib/lib/corpus/unicode-cldr.generated"
ACTIVE="$ROOT/stdlib/lib/corpus/unicode-cldr-current-chunk.generated.px"
export PNIX_EVAL_MAX_DEPTH="${PNIX_EVAL_MAX_DEPTH:-300000}"
if [[ ! -d "$CHUNK_DIR" ]]; then "$ROOT/scripts/gen-text-unicode-cldr.sh"; fi
count=0
...
rm -f "$ACTIVE"
echo "unicode-cldr redb load complete: chunks=$count"
