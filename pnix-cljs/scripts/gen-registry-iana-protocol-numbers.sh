#!/usr/bin/env bash
# IANA Protocol Numbers XML -> pnix attrset source. 기관 XML registry columns 보존, redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-protocol-numbers"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-protocol-numbers.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; name = "%s"; description = "%s"; ipv6 = "%s"; xrefs = %s; }' % (
      esc(child_text(rec,'value')), esc(child_text(rec,'name')), esc(child_text(rec,'description')), esc(child_text(rec,'ipv6')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-protocol-numbers.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-protocol-numbers.sh')
lines.append('{')
lines.append('  schema = "registry.iana.protocol_numbers.v1";')
lines.append('  source = {')
...
lines.append('  };')
lines.append('  registry = { id = "protocol-numbers-1"; title = "Assigned Internet Protocol Numbers"; records = [')
for rec in records:
    lines.append('    %s' % record_literal(rec))
lines.append('  ]; };')
lines.append('  registry_count = 1;')
lines.append('  record_count = %d;' % len(records))
lines.append('  generated_note = "IANA Protocol Numbers XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
