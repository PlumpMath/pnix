#!/usr/bin/env bash
# IANA EAP Numbers XML 최신 snapshot 수집. redb 적재는 하지 않는다.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-eap-numbers"
URL="https://www.iana.org/assignments/eap-numbers/eap-numbers.xml"
mkdir -p "$SRC_DIR"
TMP="$SRC_DIR/eap-numbers.xml.tmp"
OUT="$SRC_DIR/eap-numbers.xml"
curl -fsSL "$URL" -o "$TMP"
mv "$TMP" "$OUT"
...
cat > "$SRC_DIR/manifest.json" <<JSON
{
  ...
  "source_id": "iana-eap-numbers",
  "project": "IANA EAP Numbers",
