#!/usr/bin/env bash
# IANA TLS Parameters XML -> pnix attrset source. TLS registry columns preserved flat, redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-tls-parameters"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-tls-parameters.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; dtls = "%s"; recommended = "%s"; "rec" = "%s"; xrefs = %s; }' % (
        esc(child_text(rec,'value')), esc(child_text(rec,'dtls')), esc(child_text(rec,'recommended')), esc(child_text(rec,'rec')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-tls-parameters.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-tls-parameters.sh')
lines.append('{')
lines.append('  schema = "registry.iana.tls_parameters.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA TLS Parameters flat registry transcription only; description/comment prose excluded; no graph/mirror/math wiring";')
lines.append('}')
