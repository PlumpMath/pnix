#!/usr/bin/env bash
# IANA SASL Mechanisms XML 최신 snapshot 수집. redb 적재는 하지 않는다.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-sasl-mechanisms"
URL="https://www.iana.org/assignments/sasl-mechanisms/sasl-mechanisms.xml"
mkdir -p "$SRC_DIR"
TMP="$SRC_DIR/sasl-mechanisms.xml.tmp"
OUT="$SRC_DIR/sasl-mechanisms.xml"
curl -fsSL "$URL" -o "$TMP"
mv "$TMP" "$OUT"
...
cat > "$SRC_DIR/manifest.json" <<JSON
{
  ...
  "source_id": "iana-sasl-mechanisms",
  "project": "IANA SASL Mechanisms",
  ...
  "source_url": "$URL",
  "source_path": "sasl-mechanisms.xml",
  "source_sha256": "$SHA",
  "source_bytes": $BYTES
}
JSON
