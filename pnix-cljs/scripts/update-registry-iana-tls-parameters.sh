#!/usr/bin/env bash
# IANA TLS Parameters XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-tls-parameters"
URL="https://www.iana.org/assignments/tls-parameters/tls-parameters.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/tls-parameters.xml"
SHA=$(shasum -a 256 "$OUT_DIR/tls-parameters.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-tls-parameters",
  "project":"IANA TLS Parameters Registry",
  ...
  "source_url":url,
  "source_path":"tls-parameters.xml",
  "source_sha256":sha
