#!/usr/bin/env bash
# IANA CBOR Tags XML -> pnix attrset source. 기관 XML registry columns 보존, redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-cbor-tags"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-cbor-tags.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; description = "%s"; semantics = "%s"; timescale = "%s"; xrefs = %s; }' % (
      esc(child_text(rec,'value')), esc(child_text(rec,'description')), esc(child_text(rec,'semantics')), esc(child_text(rec,'timescale')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-cbor-tags.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-cbor-tags.sh')
lines.append('{')
lines.append('  schema = "registry.iana.cbor_tags.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA CBOR Tags XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
