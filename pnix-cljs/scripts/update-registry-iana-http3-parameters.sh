#!/usr/bin/env bash
# IANA HTTP/3 Parameters XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-http3-parameters"
URL="https://www.iana.org/assignments/http3-parameters/http3-parameters.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/http3-parameters.xml"
SHA=$(shasum -a 256 "$OUT_DIR/http3-parameters.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-http3-parameters",
  "project":"IANA HTTP/3 Parameters Registry",
  ...
  "source_url":url,
  "source_path":"http3-parameters.xml",
  "source_sha256":sha
