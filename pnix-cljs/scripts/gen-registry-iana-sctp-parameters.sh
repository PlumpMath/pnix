#!/usr/bin/env bash
# IANA SCTP Parameters XML -> pnix attrset source. redb 적재까지만, graph/mirror/math 연결 없음.
set -euo pipefail
...
OUT="$ROOT/stdlib/lib/corpus/iana-sctp-parameters.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-sctp-parameters.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-sctp-parameters.sh')
lines.append('{')
lines.append('  schema = "registry.iana.sctp_parameters.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA SCTP Parameters XML structural transcription only; XML child tag names preserved as field name/value pairs; no graph/mirror/math wiring";')
lines.append('}')
