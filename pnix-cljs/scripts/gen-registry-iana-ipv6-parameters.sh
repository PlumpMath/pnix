#!/usr/bin/env bash
# IANA IPv6 Parameters XML -> pnix attrset source. redb 적재까지만, graph/mirror/math 연결 없음.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-ipv6-parameters"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-ipv6-parameters.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-ipv6-parameters.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-ipv6-parameters.sh')
lines.append('{')
lines.append('  schema = "registry.iana.ipv6_parameters.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA IPv6 Parameters XML structural transcription only; XML child tag names preserved as field name/value pairs; no graph/mirror/math wiring";')
lines.append('}')
