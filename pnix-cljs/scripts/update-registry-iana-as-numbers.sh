...
OUT="$ROOT/ingest/registry/iana-as-numbers"
URL="https://www.iana.org/assignments/as-numbers/as-numbers.xml"
mkdir -p "$OUT"
curl -fsSL "$URL" -o "$OUT/as-numbers.xml"
python3 - <<'PY' "$OUT" "$URL"
import hashlib, json, os, pathlib, sys
out=pathlib.Path(sys.argv[1]); url=sys.argv[2]; p=out/'as-numbers.xml'; b=p.read_bytes()
manifest={
  'schema':'pnix.ingest.manifest.v1',
  'source_id':'iana-as-numbers',
  'project':'IANA Autonomous System Numbers',
