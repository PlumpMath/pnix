#!/usr/bin/env bash
# IANA URI Schemes XML -> pnix attrset source. 기관 XML registry columns 보존, redb 적재까지만.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
SRC_DIR="$ROOT/ingest/registry/iana-uri-schemes"
MANIFEST="$SRC_DIR/manifest.json"
OUT="$ROOT/stdlib/lib/corpus/iana-uri-schemes.generated.px"
while [ $# -gt 0 ]; do
  case "$1" in
...
def record_literal(rec):
    ...
    return '{ value = "%s"; name = "%s"; description = "%s"; status = "%s"; cri = "%s"; well_known = "%s"; dec = "%s"; hex = "%s"; bits = "%s"; controller = "%s"; notes = "%s"; xrefs = %s; }' % (
        esc(child_text(rec,'value')), esc(child_text(rec,'name')), esc(child_text(rec,'description')), esc(child_text(rec,'status')), esc(child_text(rec,'cri')), esc(child_text(rec,'well-known')), esc(child_text(rec,'dec')), esc(child_text(rec,'hex')), esc(child_text(rec,'bits')), esc(child_text(rec,'controller')), esc(child_text(rec,'notes')), list_lit(xrefs))
...
lines=[]
lines.append('# stdlib/lib/corpus/iana-uri-schemes.generated.px — GENERATED, do not commit.')
lines.append('# 생성: scripts/gen-registry-iana-uri-schemes.sh')
lines.append('{')
lines.append('  schema = "registry.iana.uri_schemes.v1";')
lines.append('  source = {')
...
lines.append('  generated_note = "IANA URI Schemes XML structural transcription only; no graph/mirror/math wiring";')
lines.append('}')
