#!/usr/bin/env bash
# IANA HTTP Status Code Registry XML snapshot downloader. No graph/mirror/math wiring.
set -euo pipefail
ROOT="${PNIX_WORKSPACE_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
OUT_DIR="$ROOT/ingest/registry/iana-http-status-codes"
URL="https://www.iana.org/assignments/http-status-codes/http-status-codes.xml"
mkdir -p "$OUT_DIR"
curl -fsSL "$URL" -o "$OUT_DIR/http-status-codes.xml"
SHA=$(shasum -a 256 "$OUT_DIR/http-status-codes.xml" | awk '{print $1}')
...
json.dump({
  ...
  "source_id":"iana-http-status-codes",
  "project":"IANA HTTP Status Code Registry",
  ...
  "source_url":url,
  "source_path":"http-status-codes.xml",
  "source_sha256":sha
