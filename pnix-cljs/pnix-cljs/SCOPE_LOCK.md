...
이식 가능한 언어 의미는 저장소 수준 `pnix-meta`에 속한다. 이 호스트는
복사된 Clojure/JVM 런타임 트리를 유지하기보다 그 의미를 소비해야 한다.
네이티브 seed는 공유 적합 코퍼스가 연결되고 all-host gate로 비교되기 전까지
정규 크로스호스트 패리티를 주장할 수 없다.
