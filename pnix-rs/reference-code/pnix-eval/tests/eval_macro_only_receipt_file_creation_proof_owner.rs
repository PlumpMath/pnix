...
fn fixture_path() -> PathBuf {
  Path::new(env!("CARGO_MANIFEST_DIR"))
    .join("../../fixtures/pnix-query-runtime/macro-only-receipt-file-creation-proof-owner.px")
}
...
fn eval_fixture() -> &'static Value {
  ...
  VALUE.get_or_init(|| {
    ...
    let json = std::thread::Builder::new()
      .name("macro-only-receipt-file-creation-proof-owner-eval".to_string())
      .stack_size(32 * 1024 * 1024)
      .spawn(move || {
        eval_to_json(path.to_str().expect("utf-8 path"), true)
          .expect("receipt file creation proof owner")
      })
...
#[test]
fn owner_fixture_uses_disk_write_source_and_stage() {
...
#[test]
fn valid_proof_emits_five_file_creation_proof_records() {
  let run = eval_fixture();
...
#[test]
fn file_creation_records_preserve_path_and_remain_proof_only() {
  let run = eval_fixture();
  let proof = get(run, "valid-proof");
  let records = attrs_by_id(get(proof, "file-creation-proofs"));
  let record = records[
  ...
  assert_eq!(
    ...
    "receipt-content-write-after-file-creation-proof"
  );
  let plan = get(record, "creation-plan");
  assert!(as_bool(get(plan, "dry-run-only")));
  assert!(as_bool(get(plan, "empty-file-forbidden")));
  assert!(as_bool(get(plan, "content-write-required-after-creation")));
...
fn required_sections_and_evidence_are_explicit() {
  let run = eval_fixture();
  let sections = string_set(get(run, "required-file-creation-sections"));
  for expected in [
    "source-disk-write-proof",
    "target-path-proof",
    "repo-local-path-proof",
    "parent-directory-proof",
    "file-creation-preflight-proof",
    "empty-file-forbidden-proof",
    "content-write-deferred-proof",
  ...
  for expected in [
    ...
    "one-file-creation-proof-per-disk-write-proof",
    "receipt-file-creation-proof",
    "file-creation-proof-only",
    "parent-directory-proof",
    "empty-file-forbidden-proof",
    "content-write-deferred-proof",
...
#[test]
fn top_level_fixture_records_file_creation_proof_only() {
  let run = eval_fixture();
  assert!(as_bool(get(run, "receipt-file-creation-proof")));
  assert!(as_bool(get(run, "file-creation-proof-only")));
  assert_eq!(as_i64(get(run, "file-creation-proof-count")), 5);
  assert_eq!(as_i64(get(run, "source-disk-write-proof-count")), 5);
  for key in [
    "receipt-file-created",
    "receipt-content-written",
    "receipt-auto-written",
