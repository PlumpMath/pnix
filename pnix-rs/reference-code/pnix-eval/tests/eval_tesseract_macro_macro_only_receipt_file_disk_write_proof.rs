...
fn fixture_path() -> PathBuf {
  Path::new(env!("CARGO_MANIFEST_DIR")).join(
    "../../fixtures/tesseract-macro-legacy-probe/macro_only_receipt_file_disk_write_proof_receipt.px",
  )
}
...
fn eval_fixture() -> &'static Value {
  ...
  VALUE.get_or_init(|| {
    ...
    let json = std::thread::Builder::new()
      .name("macro-only-receipt-file-disk-write-proof-eval".to_string())
      .stack_size(32 * 1024 * 1024)
      .spawn(move || {
        eval_to_json(path.to_str().expect("utf-8 path"), true)
          .expect("receipt file disk write proof receipt")
      })
...
#[test]
fn constitution_gate_blocks_disk_write_proof_collapse() {
  let run = eval_fixture();
  let gate = get(run, "constitutionGate");
  assert_eq!(
    as_str(get(gate, "scenario")),
    "macro-only-receipt-file-disk-write-proof"
  );
  assert_eq!(as_str(get(gate, "verdict")), "candidate-only");
  let blocks = string_set(get(gate, "blocked-shortcuts"));
  for expected in [
    "disk-write-proof-equals-actual-disk-write",
    "disk-write-proof-equals-receipt-file-created",
    "disk-write-proof-equals-receipt-content-written",
    "disk-write-proof-equals-auto-write",
    "disk-write-proof-equals-auto-approval",
    "disk-write-proof-equals-target-frontier-closed",
    "disk-write-proof-equals-delete-ready",
    "disk-write-proof-equals-implementation-command",
    "disk-write-proof-equals-global-runtime-install",
    "disk-write-proof-equals-runtime-api-flattening",
    "disk-write-proof-equals-meaning-db",
    "disk-write-proof-equals-p-puck-semantic-owner",
    "old-host-code-authorizes-disk-write",
  ] {
...
#[test]
fn proof_generates_five_disk_write_records() {
  let run = eval_fixture();
  let proof = get(run, "receipt-file-disk-write-proof");
  assert_eq!(
...
#[test]
fn disk_write_records_preserve_paths_without_file_creation_or_target_closure() {
  let run = eval_fixture();
  let records = attrs_by_id(get(run, "disk-write-proofs"));
  ...
  assert!(as_bool(get(record, "receipt-file-disk-write-proof")));
  assert!(as_bool(get(record, "disk-write-proof-only")));
  for key in [
    "disk-write-executed",
...
#[test]
fn contract_closes_disk_write_proof_only() {
  let run = eval_fixture();
  let contract = get(run, "receipt-file-disk-write-contract");
  assert_eq!(
    as_str(get(contract, "proof-id")),
    "proof.macro-only.receipt-file-disk-write.v1"
  );
  assert!(as_bool(get(contract, "closes-disk-write-proof")));
  assert_eq!(as_i64(get(contract, "disk-write-proof-count")), 5);
  for key in [
    "closes-actual-disk-write",
...
#[test]
fn migration_delta_closes_only_disk_write_proof_frontier() {
  let run = eval_fixture();
  let delta = get(run, "migrationDelta");
  let closes = string_set(get(delta, "closes"));
  assert!(closes.contains("need.self.receipt-file-disk-write-after-materialization-proof"));
  assert_eq!(closes.len(), 1);
  let not_closed = string_set(get(delta, "does-not-close"));
  for expected in [
    "need.self.receipt-file-creation-after-disk-write-proof",
...
fn trials_cover_valid_source_proof_shape_and_held_boundaries() {
  let run = eval_fixture();
  let trials = attrs_by_id(get(run, "receipt-file-disk-write-trials"));
  assert_eq!(trials.len(), 20);
  assert_eq!(
    as_str(get(trials["trial.A.valid-disk-write-proof"], "outcome")),
    "receipt-file-disk-write-proof-present"
  );
  for (id, held) in [
    (
    ...
    (
      "trial.F.materialization-missing",
      "held.macro-only-receipt-file-disk-write.materialization-proof-missing",
    ),
    (
      "trial.G.materialization-count-mismatch",
    ...
    (
      "trial.I.source-materialization-overclaim",
...
#[test]
fn discoveries_record_d572_through_d579() {
  let run = eval_fixture();
  let discoveries = attrs_by_id(get(run, "discoveries"));
  assert_eq!(discoveries.len(), 8);
  for expected in [
    "D572.disk-write-proof-is-separate-from-actual-disk-write",
    "D573.disk-write-proofs-preserve-materialization-targets",
    "D574.one-disk-write-proof-per-materialization-keeps-five-lane-split",
    "D575.dry-run-boundary-precedes-file-creation",
    "D576.disk-write-proof-record-is-not-written-receipt-content",
    "D577.disk-write-proof-opens-file-creation-frontier",
    "D578.disk-write-hard-stops-block-runtime-and-delete-collapse",
    "D579.disk-write-proof-keeps-approval-and-target-frontier-separate",
  ] {
...
#[test]
fn top_level_flags_keep_actual_write_and_runtime_false() {
  let run = eval_fixture();
  assert!(as_bool(get(run, "receipt-file-disk-write-proof-present")));
