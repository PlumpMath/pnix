...
echo "# px 값을 Rust 프로그램으로 사영 -> substrate 3-way lossless"
run rust-mirror -c '{ a = 1; b = [ 2 3 ]; sum = 2 + 3; }' | grep -E "status|program_sha256"
echo
echo "# 값 축(3-way) + AST 축(Rust->px->Rust 재구성, ast 동일성 + rustc 정합)"
run rust-mirror-check | tail -3
