...
돌려준다. 2026-08-21 기준 5개 호스트 모두 이 모양 (인라인 파일 라벨
`"<pnix-px>"`, 생성 attrset은 `null`, `inherit` 이름은 inherit 절 위치).
**rs(여기)** 는 2026-08-20에 hy 오라클과 같은 모양, 2026-08-21에 inherit
위치와 `-f` 파일 eval의 실제 경로(`px_parse_in`)를 맞춤.
