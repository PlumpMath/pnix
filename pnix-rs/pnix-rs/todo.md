...
- 2026-08-19 5개 호스트 감사 결과:
  ...
  - cljs: 빌트인 이름은 등록돼있는데 호출하면 "not-callable" 에러 — 이름만
    있고 실제 구현이 없는 죽은 항목.
  - **rs (여기)**: `functionArgs`/`substring`/`.${...}` 등 오늘 40개 빌트인
    추가로 많은 게 채워졌지만 `unsafeGetAttrPos`는 아직 등록조차 안 됨.
- 방향 아이디어(확정 아님): line/column 추적은 이 빌트인 하나만을 위한 게
  ...
  개선 작업과 묶는 게 나을 수 있다. `import`가 2026-08-19에 실제 파일시스템
  읽기로 열렸으니(rs도 이미 파일 모드 `-f`로 잘 동작함), "어느 파일인지"
  추적하는 것도 이제 실제로 의미가 생겼다.
