    ...
    in
    {
      ...
      packages = forAll (pkgs:
        let
          ...
          # Host-library path printer (ShellCheck-clean: no bare ls).
          pnix-rs-refs = pkgs.writeShellApplication {
            name = "pnix-rs-refs";
            text = ''
              lib="${pnixRsLibrary}"
              ...
              if [ -d "$lib/include" ]; then
                find "$lib/include" -maxdepth 1 -type f -printf '#   include/%f\n' 2>/dev/null \
                  || find "$lib/include" -maxdepth 1 -type f | sed 's|.*/|#   include/|'
              fi
            '';
          };
        in
      ...
      apps = forAll (pkgs:
        ...
        in
        {
          ...
          pnix-rs-refs = {
            type = "app";
            program = "${p.pnix-rs-refs}/bin/pnix-rs-refs";
          };
          # checks
